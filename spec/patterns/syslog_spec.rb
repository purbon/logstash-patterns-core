...
describe "SYSLOGLINE" do
  ...
  it "matches a simple message with pid" do
    value = "May 11 15:17:02 meow.soy.se CRON[10973]: pam_unix(cron:session): session opened for user root by (uid=0)"
    expect(value).to match(subject)
  end

  it "matches prog with slash" do
    value = "Mar 16 00:01:25 evita postfix/smtpd[1713]: connect from camomile.cloud9.net[168.100.1.3]"
    expect(value).to match(subject)
  end

  it "matches prog from ansible" do
    value = "May 11 15:40:51 meow.soy.se ansible-<stdin>: Invoked with filter=* fact_path=/etc/ansible/facts.d"
    expect(value).to match(subject)
  end
  ...
  it "matches prog from RFC5424 APP-NAME" do
    ...
    value        = "May 11 15:40:51 meow.soy.se #{tag_from_rfc}: Just some data which conforms to RFC5424"
    expect(value).to match(subject)
  end
